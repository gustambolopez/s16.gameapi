...
app.get("/v0/api/games/q=:searchTerm", async (req, res) => {
  ...
  try {
    ...
    if (!resp.ok) {
      ...
      return res
        .status(resp.status)
        .json({ error: `Fetch failed: ${errText}` });
    }
    ...
    if (data.errors) {
      ...
      return res
        .status(500)
        .json({ error: "GraphQL errors", details: data.errors });
    }

    const results = Array.isArray(data?.data?.gamesSearched?.hits)
      ? await Promise.all(
          data.data.gamesSearched.hits.map((result) =>
            theresultlmao({
              id: result.objectID,
              ...result,
...
app.get("/v0/api/games/:id", async (req, res) => {
  ...
  try {
    ...
    if (gameResult) {
      ...
      res
        .status(404)
        .json({ error: "Game not found, is the ID correct or does it exist?" });
    }
  } catch (err) {
    console.error("Fetch error:", err);
    res.status(500).json({ error: "Server error", details: err.message });
